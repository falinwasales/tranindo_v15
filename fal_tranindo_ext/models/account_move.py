    ...
    def _generate_efaktur_invoice(self, delimiter):
        ...
        for move in self.filtered(lambda m: m.state == 'posted'):
            ...
            for line in move.line_ids.filtered(lambda l: not l.exclude_from_invoice_tab and not l.display_type):
                ...
                harga_total = invoice_line_total_price / (100/100 + sum(tax.amount/100 for tax in line.tax_ids)) if line.tax_ids.mapped('price_include') else invoice_line_total_price
                discount_value = harga_total * line.discount/100
                dpp_amount = harga_total - discount_value
                ppn_amount = (harga_total - discount_value) * sum(tax.amount/100 for tax in line.tax_ids)
