...
class AccountInvoice(models.Model):
    ...
    #         for line in self.invoice_line_ids.filtered(lambda l: not l.exclude_from_invoice_tab and not l.display_type):
    ...
    #                 'DPP': int(line.price_subtotal - (total-(line.discount/100))),
    ...
    #                     'PPN': int(float_round(line.tax_ids.amount, 0)),
    #                 })
    #                 free.append(line_dict)
    ...
    def _generate_efaktur_invoice(self, delimiter):
        ...
        for move in self.filtered(lambda m: m.state == 'posted'):
            ...
            for line in move.line_ids.filtered(lambda l: not l.exclude_from_invoice_tab and not l.display_type):
                ...
                discount_value = round(harga_total * line.discount/100)
                ...
                line_dict = {
                    ...
                    'HARGA_SATUAN': int(float_round(invoice_line_unit_price, 0)),
                    'JUMLAH_BARANG': line.quantity,
                    'HARGA_TOTAL': int(round(harga_total)),
                    'DPP': int(round(harga_total - discount_value)),
                    'product_id': line.product_id.id,
                }
